...
void CPMDataTransformer<Dtype>::Transform_nv(const Datum& datum, Dtype* transformed_data, Dtype* transformed_label, int cnt) {
  ...
  //Mat mask_miss_aug = cv::Mat::zeros(crop_y, crop_x, CV_8UC1);
  //Mat mask_all_aug = cv::Mat::zeros(crop_y, crop_x, CV_8UC1);
  ...
  if (phase_ == TRAIN) {
    ...
    // cv::addWeighted(label_map, 0.5, img_aug, 0.5, 0.0, label_map);
    ...
    if (mode > 5){
      cv::resize(mask_all_aug, mask_all_aug, cv::Size(), 1.0/stride, 1.0/stride, cv::INTER_CUBIC);
    }
  }
  else {
    img_aug = img.clone();
...
// include mask_miss
template<typename Dtype>
float CPMDataTransformer<Dtype>::augmentation_scale(cv::Mat& img_src, cv::Mat& img_temp, cv::Mat& mask_miss, cv::Mat& mask_all, MetaData& meta, int mode) {
  ...
  if(mode>5){
    cv::resize(mask_all, mask_all, cv::Size(), scale, scale, cv::INTER_CUBIC);
  }
...
template<typename Dtype>
cv::Size CPMDataTransformer<Dtype>::augmentation_croppad(cv::Mat& img_src, cv::Mat& img_dst, cv::Mat& mask_miss, cv::Mat& mask_miss_aug, cv::Mat& mask_all, cv::Mat& mask_all_aug, MetaData& meta, int mode) {
  ...
  //LOG(INFO) << "ROI is " << x_offset << " " << y_offset << " " << std::min(800, img_temp.cols) << " " << std::min(256, img_temp.rows);
...
template<typename Dtype>
bool CPMDataTransformer<Dtype>::augmentation_flip(cv::Mat& img_src, cv::Mat& img_aug, cv::Mat& mask_miss, cv::Mat& mask_all, MetaData& meta, int mode) {
  bool doflip;
...
template<typename Dtype>
float CPMDataTransformer<Dtype>::augmentation_rotate(cv::Mat& img_src, cv::Mat& img_dst, cv::Mat& mask_miss, cv::Mat& mask_all, MetaData& meta, int mode) {
  ...
  if(mode >4){
    warpAffine(mask_miss, mask_miss, R, bbox.size(), cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(255)); //Scalar(0) for MPI, COCO with cv::Scalar(255);
  }
  if(mode >5){
    warpAffine(mask_all, mask_all, R, bbox.size(), cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(0));
  }
...
template<typename Dtype>
float CPMDataTransformer<Dtype>::augmentation_scale(cv::Mat& img_src, cv::Mat& img_temp, MetaData& meta) {
  float dice = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
  float scale_multiplier;
  //float scale = (param_.scale_max() - param_.scale_min()) * dice + param_.scale_min(); //linear shear into [scale_min, scale_max]
  if(dice > param_.scale_prob()) {
...
template<typename Dtype>
cv::Size CPMDataTransformer<Dtype>::augmentation_croppad(cv::Mat& img_src, cv::Mat& img_dst, MetaData& meta) {
  float dice_x = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
  float dice_y = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
  int crop_x = param_.crop_size_x();
  int crop_y = param_.crop_size_y();
...
template<typename Dtype>
bool CPMDataTransformer<Dtype>::augmentation_flip(cv::Mat& img_src, cv::Mat& img_aug, MetaData& meta) {
  bool doflip;
  if(param_.aug_way() == "rand"){
    float dice = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
    doflip = (dice <= param_.flip_prob());
...
template<typename Dtype>
float CPMDataTransformer<Dtype>::augmentation_rotate(cv::Mat& img_src, cv::Mat& img_dst, MetaData& meta) {
...
template<typename Dtype>
void CPMDataTransformer<Dtype>::putVecPeaks(Dtype* entryX, Dtype* entryY, cv::Mat& count, cv::Point2f centerA, cv::Point2f centerB, int stride, int grid_x, int grid_y, float sigma, int thre){
  //int thre = 4;
...
template<typename Dtype>
void CPMDataTransformer<Dtype>::visualize(cv::Mat& img, MetaData meta, AugmentSelection as) {
